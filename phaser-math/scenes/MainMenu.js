...
export default class MainMenu extends Phaser.Scene {
    ...
    create() {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            rect.fillStyle(0x000000, 0.8); // Black color with 80% opacity
            rect.fillRect(x - 50, y - 20, 100, 40); // Position and size of the rectangle
            ...
            // Add button hover and click events
            levelText.on('pointerover', () => rect.fillStyle(0xbaced0, 1).fillRect(x - 50, y - 20, 100, 40)); // Deep blue color on hover
            levelText.on('pointerout', () => rect.fillStyle(0x000000, 0.8).fillRect(x - 50, y - 20, 100, 40));  // Revert to black with 80% opacity on hover out
            levelText.on('pointerdown', () => this.startLevel(i)); // Start the corresponding level on click
        }


        // Add instructions text
        this.add.text(this.scale.width / 2, this.scale.height / 2 + 120, 'Easy peasy maths', { fontSize: '32px', fill: '#fff' }).setOrigin(0.5);
    }
